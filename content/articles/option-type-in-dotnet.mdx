...
Null reference exceptions are the most preventable bugs in software. Tony Hoare, who invented the null reference in 1965, later called it his "billion-dollar mistake." Decades on, most codebases still do not treat it as the design problem it is.

I kept hitting the same wall in production: `NullReferenceException` incidents that had sailed through code review, because nothing in the types said a value might be missing. The fix did not come from .NET. It came from a language that refuses to have the problem at all.
...
That is the whole type. There is no secret third state, no "present but actually null." It is what's called a *sum type*: a value that is exactly one of a fixed set of shapes, and the type spells out which shapes are allowed.

The payoff is in how you read it back. To get at the value, you *pattern match* — you write a branch for each possible shape:
...
So I built one. [Waystone.Monads](https://github.com/draekien-industries/waystone-dotnet/tree/main/src/Waystone.Monads) is a direct port of Rust's `std::option` to .NET. Same type, same two states, same idea: absence is a value you must handle, not a landmine you might step on.
...
C# has no `match` keyword, so the library gives you a `Match` method that plays the same role:
...
`Map` transforms the inner value *if it exists*, and leaves `None` untouched:
...
`FlatMap` chains steps that *themselves* return an `Option`. Rust calls this `and_then`; the idea is identical:
