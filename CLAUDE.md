...
**Commits** — make git commits via the Bash tool with a quoted heredoc (`git commit -F - <<'EOF'`). The PowerShell tool's `@'...'@` here-string leaks literal `@` delimiters into the commit body.
